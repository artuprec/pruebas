...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 24 11:21:32 2022
...
class Monitor():
    def __init__(self):
        ...
        self.south_passed = Value('i',0)
        self.north_passed = Value('i',0)
        self.south_inside = Value('i',0)
        self.north_inside = Value('i',0)
        self.free_tunnel_north=Condition(self.mutex)
        self.free_tunnel_south= Condition(self.mutex)

    def is_free_tunnel_north(self):
        return ((self.north_passed.value < LIM and self.north_waiting.value >= self.south_waiting.value) or self.south_waiting.value == 0) and self.south_inside.value == 0

    def is_free_tunnel_south(self):
        return ((self.south_passed.value < LIM and self.south_waiting.value >= self.north_waiting.value)  or self.north_waiting.value == 0) and self.north_inside.value == 0
    ...
    def leaves_tunnel(self, direction):
        ...
        if direction == NORTH:
            self.north_inside.value -= 1 
            self.south_passed.value = 0
            if self.north_passed.value <= LIM or self.south_waiting.value != 0 :
